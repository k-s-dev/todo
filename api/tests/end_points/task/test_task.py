...
class TaskApiMinimalSetupClass(CustomTestCaseSetup):
    ...
    def setUpTestData(cls):
        ...
        cls.view_name_list = "api:task-list"
        cls.view_name_detail = "api:task-detail"
    ...
    @classmethod
    def get_task_serializers(cls, pk_seq=None, cat_pk_seq=None, pr_pk_seq=None):
        task_query = cls.get_task_query(
            pk_seq=pk_seq, cat_pk_seq=cat_pk_seq, pr_pk_seq=pr_pk_seq).order_by("id")
...
class TaskApiAuthAndCrudTests(TaskApiFullSetupTestClass):
    ...
    def test_auth_required(self):
        ...
        response = self.client.get(reverse(
            self.view_name_list,
            kwargs={
                "ws_pk": self.cat_1_task_1.category.workspace.pk,
                "cat_pk": self.cat_1_task_1.category.pk,
            }))
    ...
    def test_task_get(self):
        ...
        cat_1_serializers = self.get_task_serializers(
            cat_pk_seq=[self.cat_1_task_1.category.pk])
...
class TaskApiTreeTests(TaskApiFullSetupTestClass):
    ...
    def test_create_post(self):
        ...
        url = reverse(self.view_name_list,
                      kwargs={
                          "ws_pk": self.cat_1_nested_task_1_1.category.workspace.pk,
                          "cat_pk": self.cat_1_nested_task_1_1.category.pk,
                      })
        response = self.client.post(url, data)
        retrieved_data = core_models.Task.objects.filter(
